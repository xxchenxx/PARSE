...
def embed_protein_custom_with_target_resid(atom_df, model_fn, pdb_id, change_resid, change_aa, change_chain, model, start_residue, device='cpu', include_hets=True, env_radius=10.0, uniprot=None):
    ...
    # print("Start of the function", change_resid, change_aa, change_chain)
    current_chain = None
    current_resid = None
    ...
    with open(args.output_name, "a+") as f:
        for pdb, resid in zip(db['pdbs'], db['resids']):
            ...
            try:
                ...
                rnk.set_index('site', inplace=True)
                changed_results[f"{pdb}_{chain}_{pos}"] = rnk
                
                dropped[f"{pdb}_{chain}_{pos}"] = original_results[pdb]['score'] - changed_results[f"{pdb}_{chain}_{pos}"]['score']
                mean_score_with_the_pdb = 0
                count = 0
                for i in dropped[f"{pdb}_{chain}_{pos}"].index:
                    if i.startswith(pdb):
                        # print(dropped[f"{pdb}_{pos}"].loc[i])
                        mean_score_with_the_pdb += dropped[f"{pdb}_{chain}_{pos}"].loc[i]
                        count += 1
                # print(mean_score_with_the_pdb / count)
                f.write(f"{pdb}_{chain}_{pos} {mean_score_with_the_pdb / count}\n")
                f.flush()
            except:
