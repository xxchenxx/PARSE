...
if __name__=="__main__":
    ...
    for pdb in glob(args.pdb_dir + "/*.pdb"):
        if os.path.exists(f"{args.output_dir}/ours_rnk_{pdb.split('/')[-1].split('.')[0]}.csv"):
            continue
        # model = initialize_model(device=device)
        try:
            pdb_df = process_pdb(pdb, chain=args.chain, include_hets=False)
        except:
            continue
        if not 'AF' in pdb:
            embed_data = embed_protein_custom(model, pdb_df, None, pdb.split("/")[-1].split(".")[0], device, include_hets=False)
        else:
            try:
                embed_data = embed_protein_custom(model, pdb_df, None, pdb.split("/")[-1].split(".")[0], device, include_hets=False, uniprot=pdb.split("-")[1])
            except:
                embed_data = None
        ...
        if embed_data is not None:
            ...
            rnk.to_csv(f"{args.output_dir}/ours_rnk_{pdb_id}.csv")
